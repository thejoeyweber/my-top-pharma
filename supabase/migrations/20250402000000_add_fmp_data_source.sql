...
DO $$ 
BEGIN
    -- Check if is_active column exists in data_source_endpoints
    IF NOT EXISTS (
        SELECT FROM information_schema.columns 
        WHERE table_name = 'data_source_endpoints' AND column_name = 'is_active'
    ) THEN
        -- Add the is_active column
        ALTER TABLE public.data_source_endpoints ADD COLUMN is_active BOOLEAN DEFAULT true;
    END IF;
END $$;
